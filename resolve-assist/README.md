...
- **お手本スタイル学習** — 既成の動画から編集の型(カットテンポ・字幕体裁・構成・音量感)を
  プロファイル化し、新しい動画に自動適用
- **ショート動画生成** — 長い動画から見どころを自動提案し、字幕焼き込み済みの縦型 9:16 動画
  (YouTube Shorts / Instagram Reels にそのままアップ可能) を書き出し
...
## ショート動画生成 (YouTube Shorts / Instagram Reels)

長い動画(レビュー動画など)から、そのままアップできる縦型 1080×1920 の mp4 を生成します。

```bash
# 見どころを自動提案して3本生成 (無音カットで詰めて60秒以内に)
resolve-assist shorts review.mp4

# Reels 向けに90秒上限・2本
resolve-assist shorts review.mp4 --max-duration 90 --count 2

# 区間を自分で指定 (複数可)。字幕焼き込みなし
resolve-assist shorts review.mp4 --range 2:15-3:10 --range 5:00-5:45 --no-burn

# 中央クロップで迫力重視 (既定はぼかし背景)
resolve-assist shorts review.mp4 --fit crop
```

GUI では「ショート動画」タブから同じことができます。

- **見どころの自動提案**: 文字起こしを解析し、話速(情報密度)・キーワード(「おすすめ」
  「ポイント」「結論」等)・感嘆表現でスコアリング。ショートは冒頭の掴みが命なので、
  面白い発言から始まる区間を優先します
- **無音カット内蔵**: 選ばれた区間内も無音を詰めてテンポよく仕上げます
- **字幕焼き込み**: ショート定番の大きめ白文字+黒縁を下部に焼き込み(BudouX の自然な改行)。
  無音視聴の多い SNS で効果的。`--no-burn` で無効化でき、その場合も編集用の SRT は
  各クリップごとに出力されます
- **フォント**: 既定は macOS の Hiragino Sans。`--font "名前"` で変更できます
- 各クリップの元区間・尺・スコアは `shorts_summary.txt` で確認できます
