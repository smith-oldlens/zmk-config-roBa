...
## 人物の用意のしかた（3通り・課金なしで使えます）

1. **手持ちの画像を取り込む（最も確実・完全無料・APIキー不要）**
   - 「✨人物を生成」画面の **「🖼 手持ちの画像を取り込む」** から画像を選ぶだけ
   - 切り抜き済みの透過PNGはそのまま使えます。背景つきの写真は「切り抜きの調整」→「背景色を選び直す」で背景をスポイト指定して抜けます
2. **Pollinations で生成（無料・APIキー不要）**
   - 既定のサービス。プロンプトから自動生成。※ 提供元の状況により一時的に `403` 等で使えないことがあります
3. **Google Gemini で生成（高品質・要APIキー＆有料枠）**
   - ⚙設定で選択。画像生成に有料枠（お支払い設定）の有効化が必要（無料枠だと `429`）

## 使い方

1. **人物を用意** — 上記のいずれかの方法で（迷ったら 1 の「取り込み」が確実）
