...
> `bps finalize` / `bps calibrate` が動作し、200 枚(1200x800)を 3.6 秒で処理。
>
> **§6.2 の RTMDet-nano は未着手**(実行環境からモデル配布元へ到達できないため)。
> ただし **M0 の実機確認で AF 枠が取れると判明したため、検出器なしでも被写体領域は確定できる**:
> ingest 時に `FocusLocation` + `FocusFrameSize` を読んで `af_json` に保存し、
> 採点時にその位置を主被写体として測る(`subject_source: "af_box"`)。
> AF が無いフレームだけが従来の中央 40% クロップに落ちる(`"no_detector"`)。
>
> 効果の実測(被写体ボケ+背景シャープの合成画像): AF ありなら raw 1.19 vs 3.67 と
> 3 倍以上の差がつくが、中央クロップだと 2.53 vs 3.14 に潰れてブレを見逃す。
> 検出器を足せばさらに精度が上がるが、**現状でも「背景ボケと被写体ブレの区別」は機能する**。
