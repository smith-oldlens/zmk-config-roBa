...
依存(すべて CPU で動作すること): `opencv-python-headless`, `onnxruntime`,
`pyexiftool`, `watchdog`, `pyyaml`, `piexif`(EXIF 高速読み取り), `numpy`, `scikit-learn`(Phase 3)。
exiftool 本体はシステムインストール(パスは config で指定可)。

**モデルのライセンス方針(製品化制約・docs/05 参照)**: ランタイム依存は Apache/MIT/BSD 系のみ。
**AGPL 系(Ultralytics YOLO 含む)は使用禁止**(エクスポート済み重みにも AGPL が及ぶという
Ultralytics の主張があり、販売製品で係争リスクになるため)。採用モデル:
- 人物検出: **RTMDet-nano/tiny**(Apache-2.0, ONNX)
- ポーズ: **RTMPose-m**(Apache-2.0, ONNX。CPU+ONNX Runtime で 90+ FPS 実証)
- 埋め込み: **SigLIP2-base**(Apache-2.0)を ONNX 化して同梱。※SigLIP はゼロショット分類が
  弱いが、本設計は学習済み線形分類器の特徴抽出にのみ使うため問題ない
- 推論はすべて onnxruntime。**torch / open_clip は開発機でのモデル ONNX 化・教師モデル作成にのみ
  使用し(scripts/ 配下)、ランタイム依存・配布物には含めない**。顧客 PC 内での追加学習
  (パーソナライズ)は埋め込み(ONNX)+ scikit-learn(BSD)のみで完結する
...
├── models/       # rtmdet_nano.onnx / rtmpose_m.onnx / siglip2_base.onnx / moment_classifier.pkl
...
1. RTMDet-nano(ONNX, 入力 640, conf 0.25, class=person のみ)で人物 bbox 群を取得。
...
- グループ間: SigLIP2-base(ONNX)の画像埋め込み → 学習済みロジスティック回帰
...
- config の `af.tag_names`(M0 で実機確認して確定。既定候補順:
  ...
  を順に試し、最初に取れた値を解釈する。
- `FocusLocation` は "W H X Y" 形式(画像幅・高さ・焦点座標、左上原点)。パース結果と生文字列の
  両方を af_json に保存(後から解釈を修正できるように)。
- **中心フォールバック判別(重要)**: Sony の仕様上、AF 座標が取得できなかったフレームでは
  FocusLocation に**画像中心の座標が入る**。座標が画像中心(W/2, H/2)から
  ±1% 以内の場合は `af_json` に `"center_suspect": true` を記録し、主被写体特定(§6.2)では
  af_point=None(フォールバック経路)として扱う。本物の中央 AF も巻き込まれるが、
  中央加重フォールバックの挙動は中央 AF とほぼ同じ結果になるため実害はない。
...
### 9.2 推論リソース
RTMDet/RTMPose/SigLIP2 のモデルは finalize loop スレッドでのみロード・使用(シングルスレッド推論。
...
### 10.2 学習
SigLIP2-base(ONNX)埋め込み(バッチ処理、キャッシュを models/emb_cache.npz に保存)
...
- ONNX モデル(検出/pose/埋め込み)を使うテストは `@pytest.mark.slow`。CI では合成画像に対する
