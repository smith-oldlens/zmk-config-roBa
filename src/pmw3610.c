    ...
    if (x != 0 || y != 0) {
        if (input_mode == MOVE || input_mode == SNIPE) {
            ...
            if(ball_action_idx != -1) {
                ...
                int idx = -1;
                if(abs(data->ball_action_delta_x) > action_cfg.tick_x) {
                    idx = data->ball_action_delta_x > 0 ? 0 : 1;
                } else if(abs(data->ball_action_delta_y) > action_cfg.tick_y) {
...
#define BALL_ACTIONS_INST(n)                                                                       \
    ...
    static struct ball_action_cfg ball_action_cfg_##n = {                                          \
        ...
        .tick_x = DT_PROP_OR(n, tick_x, DT_PROP_OR(n, tick, CONFIG_PMW3610_BALL_ACTION_TICK)), \
        .tick_y = DT_PROP_OR(n, tick_y, DT_PROP_OR(n, tick, CONFIG_PMW3610_BALL_ACTION_TICK)),                              \
